...
const { verifyOtp } = require("../redis/otpService");
...
const receiveOTPMessage = client.on("message", async (msg) => {
  const phoneNumber = msg.from.replace("@c.us", "");
  const formattedNumber = phoneNumber.replace(/^62/, "0");
  const messageBody = msg.body.trim();

  if (/^\d{6}$/.test(messageBody)) {
    try {
      const result = await verifyOtp(`otp:${formattedNumber}`, messageBody);

      if (result.success) {
        await client.sendMessage(msg.from, "✅ Verifikasi OTP berhasil!", { sendSeen: false });
      } else {
        await client.sendMessage(msg.from, "❌ " + result.message, { sendSeen: false });
      }
    } catch (err) {
      console.error("Error saat verifikasi OTP:", err.message);
    }
  }
});
